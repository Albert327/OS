...
typedef struct
{
    float *dst;     // 输出行指针
    float *src;     // 输入行指针
    float *weights; // 权重矩阵指针
    float *biases;  // 偏置向量指针（可为 NULL）
    int in_dim;     // 输入维度
    int out_dim;    // 输出维度
} TaskNode;

static TaskNode queue[Q_SIZE];
static int rpos = 0, wpos = 0, qcnt = 0;
static int pending = 0, stopping = 0;

static mutex_t mtx = MUTEX_INIT();
static cond_t cv = COND_INIT();

static void enqueue(TaskNode job)
{
    queue[wpos] = job;
    wpos = (wpos + 1) % Q_SIZE;
    ++qcnt;
...
static void dequeue(TaskNode *job)
{
    *job = queue[rpos];
    rpos = (rpos + 1) % Q_SIZE;
    --qcnt;
...
static void execute_task(const TaskNode *t)
{
    for (int o = 0; o < t->out_dim; ++o)
    {
        float acc = t->biases ? t->biases[o] : 0.0f;
        float *wrow = t->weights + o * t->in_dim;
...
void matmul_forward(float *out,
                    ...
                    float *weights,
                    float *biases,
                    int B, int T, int in_dim, int out_dim)
{
    // 启动静态线程池
